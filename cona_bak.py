...
class Weather:
    ...
    def get_data_by_date(self, date_obj):
        ...
        url = "https://lishi.tianqi.com/cuona/{}.html".format(date_str)
        try:
            r = requests.get(url, headers=headers)
            html = etree.HTML(r.text)
            lis = html.xpath("//ul[@class='thrui']/li")

            for li in lis:
                date = li.xpath("./div[1]/text()")[0].strip()
                high = li.xpath("./div[2]/text()")[0]
                low = li.xpath("./div[3]/text()")[0]
                high = float(high.rstrip("℃").strip())
                low = float(low.rstrip("℃").strip())
                temp = (high + low) / 2
